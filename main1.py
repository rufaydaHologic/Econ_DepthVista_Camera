...
# -------------------- Robust vendor DLL loader (Windows & Linux) --------------------
# Finds DepthVistaSDK.dll (or .so) in common locations and loads it; also adds folders
# to the DLL search path so dependent DLLs (e.g., OpenCV) can be found.
def load_depthvista_sdk():
    if sys.platform.startswith("linux"):
        so_name = "libDepthVistaSDK.so"
        # Try LD_LIBRARY_PATH locations and script folder
        candidates = [
            Path(__file__).resolve().parent / so_name,
            Path("/usr/local/lib") / so_name,
            Path("/usr/lib") / so_name,
        ]
        lib_path = next((str(p) for p in candidates if p.exists()), so_name)
        return ctypes.CDLL(lib_path)

    elif sys.platform == "win32":
        base = Path(__file__).resolve().parent
        candidates = [
            base / "DepthVistaSDK.dll",                              # root alongside main.py
            base / "DLL" / "x64" / "Release" / "DepthVistaSDK.dll",  # case 1
            base / "DLL" / "x64" / "Debug" / "DepthVistaSDK.dll",    # case 2
            base / "Dll" / "x64" / "Release" / "DepthVistaSDK.dll",  # case-insensitive variants
            base / "Dll" / "x64" / "Debug" / "DepthVistaSDK.dll",
        ]
        dll_path = next((p for p in candidates if p.exists()), None)
        if not dll_path:
            raise FileNotFoundError(
                "DepthVistaSDK.dll not found under SDK, DLL\\x64\\Release, or DLL\\x64\\Debug."
            )

        # Ensure dependent DLLs can be found (Python 3.8+)
        add_dir = getattr(os, "add_dll_directory", None)
        if add_dir:
            for d in {
                base,
                base / "DLL" / "x64" / "Release",
                base / "DLL" / "x64" / "Debug",
                base / "DLL" / "x64",
                base / "Dll" / "x64" / "Release",
                base / "Dll" / "x64" / "Debug",
                base / "Dll" / "x64",
                base / "Lib",  # if vendor ships extra deps here
            }:
                if d.exists():
                    add_dir(str(d))

        # Load explicitly with absolute path
        lib = ctypes.WinDLL(str(dll_path))
        print(f"Loaded DepthVistaSDK from: {dll_path}")
        return lib

    else:
        raise OSError(f"Unsupported platform: {sys.platform}")

depthVistaLib = load_depthvista_sdk()
# ------------------------------------------------------------------------------------
                            ...
                            if bool(self.allFrames.rgb.frame_data):
                                ...
                                if(lock.locked() == 0):
                                    if(self.rgb_cap == True):
                                        ...
                                        time_dt = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
                                        file_name = "DepthVista_rgb_"+time_dt+".png"
                            ...
                            if bool(self.allFrames.depth_colormap.frame_data):
                                ...
                                if(lock.locked() == 0):
                                    ...
                                    if(self.depth_cap==True):
                                        time_dt = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
                                        file_name = "DepthVista_Depth_"+time_dt+".bmp"
                                        ...
                                        file_name = "DepthVista_Raw_"+time_dt+".raw"
                                        ply_file_name = "DepthVista_PLY_"+time_dt+".ply"
                            ...
                            if bool(self.allFrames.ir.frame_data):
                                ...
                                if actual_size == expected_size:
                                    ...
                                    if(lock.locked() == 0):
                                        ...
                                        if(self.IR_cap == True):
                                            time_dt = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
                                            file_name = "DepthVista_IR_"+time_dt+".png"
